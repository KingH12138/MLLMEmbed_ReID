...
class CLIP(nn.Module):
    ...
    def load_param(self, state_dict, pretrained_load=True, pretrain_load_visual=True):
...
def build_CLIP_from_openai_pretrained(name: str, image_size: Union[int, Tuple[int, int]], stride_size: int, jit: bool = False, download_root: str = None):
    ...
    model.load_param(state_dict, pretrained_load=False) # 不加载CLIP预训练权重，只用随机初始化
